...
void *client_func(void *c);
static int allowed=4;
int run = 1;
int close_flag=0;
extern void *select_thread(void *c);
extern int array_select[];
...
int main( int argc, char *argv[] )
{
	...
	pthread_t client[1];
	pthread_create(client,NULL,client_func,axel);
	...
	pthread_t select[1];
	pthread_create(select,NULL,select_thread,axel);
	while( !axel->ready && run )
	{
		long long int prev, done;
		
		prev = axel->bytes_done;
		axel->allowed_conn=allowed;
		//printf("\n allowed conns are %d\n",allowed);
		axel_do( axel );
	...
	printf( _("\nDownloaded %s in %s. (%.2f KB/s)\n axel->ready:%d run:%d"),
		...
		(double) axel->bytes_per_second / 1024,axel->ready,run );
	
	i = axel->ready ? 0 : 2;
	int *t;
	close_flag=1;
	pthread_join(select[0],&t);
	pthread_join(client[0],&t);
	axel_close( axel );
...
void *client_func(void *c)
{
   ...
   if(connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr))==0)
	...
	{printf("connection couldn't be established"); return ;}
   while((read(sockfd,&num_conns,sizeof(num_conns))!=EOF)&&close_flag==0)
	{
