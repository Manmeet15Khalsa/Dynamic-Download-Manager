...
void  axel_function(void *c)
   {
	...
	axel_t * axel =(axel_t *)c; 
        printf("\ninside axel function..\n");
         printf("\nready status : %d filename: %s conn_supported: %d",axel->ready,axel->filename,axel->conn[0].supported);
	char *s;
	int i, j, cur_head = 0;
	//char *s;
...
int main( int argc, char *argv[] )
{
	...
	printf("\n optind is %d argc is %d",optind,argc);
	
       /*
        for(i=0;i<(argc-optind);i++)
          {

		axel = axel_new( conf, 0, argv[optind+i] );
                printf("axel structure created and ready status : %d filename: %s conn_supported: %d",axel->ready,axel->filename,axel->conn[0].supported);
		if( axel->ready == -1 )
		{
			print_messages( axel );
			axel_close( axel );
			return( 1 );
                ...
                if( pthread_create( axel->axel_thread, NULL, &axel_function, (void *)axel ) != 0 )
		{
			perror("axel ::pthread error!!!");
			axel->ready = -1;
                        return(1);
		}
                else printf("\n thread created....");
